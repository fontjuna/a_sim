...
class Admin:
    ...
    def set_globals(self):
        ...
        work('dbm', 'set_rate', gm.수수료율, gm.세금율)

    def get_login_info(self):
        accounts = answer('api', 'GetLoginInfo', 'ACCNO')
        logging.debug(f'GetLoginInfo Accounts: {accounts}')
        gm.list계좌콤보 = accounts
        gm.config.account = accounts[0]

        gm.config.server = answer('api', 'GetLoginInfo', 'GetServerGubun')
    ...
    def get_conditions(self):
        try:
            loaded = answer('api', 'GetConditionLoad')
            if loaded: # sucess=1, fail=0
                gm.list전략튜플 = answer('api', 'GetConditionNameList')
    ...
    def set_real_remove_all(self):
        ...
        work('api', 'SetRealRemove', 'ALL', 'ALL')

    def get_holdings(self):
        logging.info('* get_holdings *')
        gm.dict잔고종목감시 = {}
        work('api', 'SetRealRemove', dc.scr.화면['실시간감시'], 'ALL')
        if gm.config.sim_no != 1: work('dbm', 'start_request_chart_data')
        self.pri_fx얻기_잔고합산()
        self.pri_fx얻기_잔고목록()
        self.pri_fx등록_종목감시()
    ...
    def com_SendRequest(self, rqname, trcode, input, output, next='0', screen=None, form='dict_list', timeout=5):
        ...
        try:
            ...
            return answer('api', 'api_request', **args)
        except Exception as e:
    ...
    def com_SendCondition(self, screen, cond_name, cond_index, search=1): # search=0: 조건검색만, 1: 조건검색 + 실시간 조건검색
        ...
        condition_list = answer('api', 'SendCondition', screen, cond_name, cond_index, search)
    ...
    def com_SendOrder(self, idx, rqname, screen, accno, ordtype, code, quantity, price, hoga, ordno, msg=None):
        ...
        name = answer('api', 'GetMasterCodeName', code)
        ...
        success = answer('api', 'SendOrder', **cmd)
    ...
    def on_fx실시간_조건검색(self, code, type, cond_name, cond_index): # 조건검색 결과 수신
        ...
        try:
            ...
            if type == 'I':
                work(f'전략{idx:02d}', 'cdn_fx편입_실시간조건감시', **job)
            elif type == 'D':
                work(f'전략{idx:02d}', 'cdn_fx이탈_실시간조건감시', **job)
        except Exception as e:
    ...
    def on_fx실시간_주식체결(self, code, rtype, dictFID): # 실시간 시세 감시, 시장 체결데이타 분석 재료, 종목의 누적 거래향
        ...
        if updated:
            ...
            if data:
                ...
                if data['kind'] == '매수':
                    work(f'전략{data["idx"]:02d}', 'order_buy', code, f'전략{data["idx"]:02d}', 현재가)
                elif data['kind'] == '매도':
                    row = gm.잔고목록.get(key=code)
                    row['현재가'] = 현재가
                    work(f'전략{data["idx"]:02d}', 'order_sell', row, True) # 조건검색에서 온 것이기 때문에 True
    ...
    def pri_fx얻기_잔고목록(self):
        try:
            ...
            def save_counter(dict_list):
                data = {}
                for item in dict_list:
                    전일가 = answer('api', 'GetMasterLastPrice', item['종목번호'])
                    ...
                    # work('cdr', 'register_code', item['종목번호'])
    ...
    def pri_fx검사_매도요건(self, idx, code, 전략):
        ...
        if row['현재가'] == 0: return
        if row['상태'] == 0: return
        if 전략 not in workers.keys(): return
        key = f'{code}_매도'
    ...
    def pri_fx등록_종목감시(self):
        try:
            ...
            work('api', 'SetRealReg', dc.scr.화면['실시간감시'], codes, "10", 0)
        except Exception as e:
    ...
    def pri_fx얻기_매매목록(self, date_text):
        try:
            gm.매매목록.delete()
            #dict_list = answer('dbm', 'execute_query', sql=dc.ddb.TRD_SELECT_DATE, db='db', params=(date_text,))
            dict_list = answer('dbm', 'execute_query', sql=dc.ddb.TRD_SELECT_DATE, db='db', params=(date_text,))
    ...
    def pri_fx얻기_체결목록(self, date_text):
        try:
            gm.체결목록.delete()
            #dict_list = answer('dbm', 'execute_query', sql=dc.ddb.CONC_SELECT_DATE, db='db', params=(date_text,))
            dict_list = answer('dbm', 'execute_query', sql=dc.ddb.CONC_SELECT_DATE, db='db', params=(date_text,))
    ...
    def pri_fx얻기_차트자료(self, date_text, code, cycle, tick=1):
        try:
            ...
            dict_list = answer('dbm', 'execute_query', sql=selected_sql, db='chart', params=params)
            if dict_list is not None and len(dict_list) > 0:
                if min_check:
                    dict_list = [{ **item, '일자': item['체결시간'][:8], '시간': item['체결시간'][8:], } for item in dict_list]
                else:
                    dict_list = [{ **item, '일자': item['일자'], '시간': '', '종목명': answer('api', 'GetMasterCodeName', code=item['종목코드']), } for item in dict_list]
    ...
    def cdn_fx실행_전략매매(self):
        try:
            ...
            for i in range(1, 6):
                ...
                msg = answer(f'전략{i:02d}', 'cdn_fx실행_전략매매')
    ...
    def cdn_fx중지_전략매매(self):
        try:
            for i in range(1, 6):
                work(f'전략{i:02d}', 'cdn_fx실행_전략마무리')
    ...
    def odr_timeout(self, idx, kind, origin_row, dictFID):
        ...
        try:
            ...
            work(f'전략{idx:02d}', 'order_cancel', kind, order_no, code)
    ...
    def dbm_order_upsert(self, dict_data):
        try:
            work('dbm', 'table_upsert', db='db', table='trades', dict_data=dict_data)
        except Exception as e:
    ...
    def dbm_trade_upsert(self, dictFID):
        try:
            ...
            if dictFID['주문상태'] == '체결':
                ...
                work('dbm', 'upsert_conclusion', kind, code, name, qty, price, amount, ordno, st_no, st_name, st_buy)
        except Exception as e:
    ...
    #     try:
    #         work('cdt', 'set_chart_data', code, dict_data, cycle, tick)
    #     except Exception as e:
