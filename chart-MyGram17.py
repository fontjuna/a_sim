...
class ChartUpdater:
    ...
    def get_first_chart_data(self, code, cycle, tick=1, times=1):
        ...
        try:
            ...
            if cycle in ['dy', 'mi']:
                logging.debug(f'set_chart_data: {code}, {cycle}, {tick}')
                cht_dt.set_chart_data(code, dict_list, cycle, int(tick))

                logging.debug(f'upsert_chart: {dict_list[0]}, {cycle}, {tick}')
                self.order('dbm', 'upsert_chart', dict_list, cycle, tick)

                logging.debug(f'mark_done: {code}, {cycle}')
...
class DummyWorker():
    ...
    def register_chart_data(self, job):
        code = job['code']
        if not cht_dt.is_code_registered(code): return

        dictFID = job['dictFID']
        with self.lock: 
            self.cht_updater[code] = dictFID
        logging.debug(f'register_chart_data: {code}')

    def chart_data_updater(self):
        if not self.cht_updater: return
        if time.time() - self.last_time < 5: return
        
        with self.lock:
            cht_updater_items = list(self.cht_updater.items())
            if not cht_updater_items: return
        ...
        self.last_time = time.time()
        logging.debug(f'chart_data_updater: count = {len(cht_updater_items)}')
