...
from worker import ModelThread, Order, Answer
...
class Strategy(ModelThread):
    def __init__(self, name, myq=None, ticker=None, 전략정의=None, daemon=True):
        ModelThread.__init__(self, name=name, myq=myq, daemon=daemon)
    ...
    def is_buy(self, code, rqname, price=0) -> tuple[bool, dict, str]:
        ...
        if self.매수스크립트적용:
            if self.answer(Answer(receiver='dbm', order='is_done', sender=self.전략, args=(code,))):
    ...
    def order_buy(self, code, rqname, price=0) -> tuple[bool, dict, str]:
        ...
        if is_ok:
            ...
            self.order(Order(receiver='admin', order='com_SendOrder', args=(self.전략번호,), kwargs=send_data))
        else:
    ...
    def order_sell(self, row: dict, sell_condition=False) -> tuple[bool, dict, str]:
        ...
        if is_ok:
            if not self.매도적용:
                self.order(Order(receiver='admin', order='send_status_msg', kwargs={'구분': f'매도편입', '전략': self.전략, '전략명칭': self.전략명칭, '종목코드': row['종목번호'], '종목명': row['종목명']}))
            if isinstance(send_data, list):
                logging.debug(f'** 복수 매도 주문목록 **: {send_data}')
                for data in send_data:
                    self.order(Order(receiver='admin', order='com_SendOrder', args=(self.전략번호,), kwargs=data))
            else:
                self.order(Order(receiver='admin', order='com_SendOrder', args=(self.전략번호,), kwargs=send_data))
        else:
        ...
        try:
            def run_trade(cond_index, cond_name, trade_type):
                ...
                if bool_ok:
                    ...
                    self.order(Order(receiver='admin', order='send_status_msg', args=(f'{self.전략} {trade_type} {condition}',)))
                else:
        ...
        try:
            def stop_trade(cond_index, cond_name, trade_type):
                if cond_name:
                    ...
                    self.order(Order(receiver='api', order='SendConditionStop', args=(screen, cond_name, cond_index)))
                else:
    ...
    def cdn_fx편입_실시간조건감시(self, kind, code, type, cond_name, cond_index):
        try:
            종목명 = self.answer(Answer(receiver='api', order='GetMasterCodeName', sender=self.전략, args=(code,)))
            if not self.dict종목정보.contains(code):
                전일가 = self.answer(Answer(receiver='api', order='GetMasterLastPrice', sender=self.전략, args=(code,)))
            ...
            if kind == '매도':
                ...
                if not gm.매도조건목록.in_key(code):
                    ...
                    self.order(Order(receiver='admin', order='send_status_msg', args=(f'{kind}편입', '주문내용', {'구분': f'{kind}편입', '전략': self.전략, '전략명칭': self.전략명칭, '종목코드': code, '종목명': 종목명})))
                    self.order(Order(receiver='dbm', order='register_code', args=(code,)))
    ...
    def cdn_fx이탈_실시간조건감시(self, kind, code, type, cond_name, cond_index):
        try:
            name = self.answer(Answer(receiver='api', order='GetMasterCodeName', sender=self.전략, args=(code,)))
            if kind == '매도':
            ...
            if len(gm.dict조건종목감시) > 90 and code in gm.dict조건종목감시:
                ...
                self.order(Order(receiver='api', order='SetRealRemove', kwargs={'screen': screen, 'code': code}))
    ...
    def cdn_fx등록_종목감시(self, condition_list, search_flag):
        try:
            ...
            fids = "10"  # 현재가
            self.order(Order(receiver='api', order='SetRealReg', kwargs={'screen': dc.scr.화면[self.전략], 'code_list': codes, 'fid_list': fids, 'opt_type': search_flag}))
