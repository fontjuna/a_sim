...
class Strategy:
    ...
    def is_buy(self, code, rqname, price=0) -> tuple[bool, dict, str]:
        ...
        if self.매수스크립트적용:
            if gm.ipc.admin_to_dbm('is_done', code):
        ...
        try:
            def stop_trade(cond_index, cond_name, trade_type):
                if cond_name:
                    ...
                    gm.ipc.admin_to_api('SendConditionStop', screen, cond_name, cond_index)
                else:
    ...
    def cdn_fx편입_실시간조건감시(self, kind, code, type, cond_name, cond_index):
        try:
            종목명 = gm.ipc.admin_to_api('GetMasterCodeName', code=code)
            if not self.dict종목정보.contains(code):
                전일가 = gm.ipc.admin_to_api('GetMasterLastPrice', code=code)
            ...
            if kind == '매도':
                ...
                if not gm.매수조건목록.in_key(code): 
                    ...
                    gm.ipc.admin_to_dbm('register_code', code)
    ...
    def cdn_fx이탈_실시간조건감시(self, kind, code, type, cond_name, cond_index):
        try:
            name = gm.ipc.admin_to_api('GetMasterCodeName', code=code)
            if kind == '매도':
            ...
            if len(gm.dict조건종목감시) > 90 and code in gm.dict조건종목감시:
                ...
                gm.ipc.admin_to_api('SetRealRemove', screen=screen, code=code)
    ...
    def cdn_fx등록_종목감시(self, condition_list, search_flag):
        try:
            ...
            fids = "10"  # 현재가
            gm.ipc.admin_to_api('SetRealReg', screen=dc.scr.화면[self.전략], code_list=codes, fid_list=fids, opt_type=search_flag)
