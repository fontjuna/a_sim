...
# JIT 최적화된 헬퍼 함수들
@njit
def _extremes_calculation(highs, closes, lows, volumes, amounts, n, m):
    """JIT 최적화된 극값 계산"""
    if n == 0:
    ...
    for i in range(1, min(n, len(highs))):
        ...
        if closes[i] < lc: lc = closes[i]
        if lows[i] < ll: ll = lows[i]
        if volumes[i] > hv: hv = volumes[i]
        if volumes[i] < lv: lv = volumes[i]
        if amounts[i] > ha: ha = amounts[i]
        if amounts[i] < la: la = amounts[i]
    
    return hh, hc, lc, ll, hv, lv, ha, la

@njit
def _top_selection_calculation(data, cnt):
    """JIT 최적화된 상위값 선택"""
    valid_data = []
    ...
    # 상위 cnt개 선택 (간단한 선택 정렬)
    actual_cnt = min(cnt, len(valid_data))
    
    for i in range(actual_cnt):
        max_idx = i
        for j in range(i + 1, len(valid_data)):
            if valid_data[j] > valid_data[max_idx]:
                max_idx = j
        if max_idx != i:
            valid_data[i], valid_data[max_idx] = valid_data[max_idx], valid_data[i]
    ...
    return total / actual_cnt

@njit
def _close_tops_detection(closes, n, cnt, m):
    """JIT 최적화된 최고종가 감지"""
    high_close_indices = []
    ...
    if start_idx < end_idx:
        return high_close_indices
    
    for current_idx in range(start_idx, end_idx - 1, -1):
        if current_idx >= len(closes):
            continue
        ...
        compare_end = min(current_idx + cnt, len(closes))
...
@njit
def _rsi_calculation(closes, period, m):
    """JIT 최적화된 RSI 계산"""
    if m + period + 1 > len(closes):
        return 50.0
...
@njit
def _consecutive_count_calculation(condition_results, m, max_check):
    """JIT 최적화된 연속 조건 개수 계산"""
    count = 0
    current_idx = m
    
    while current_idx < len(condition_results) and count < max_check:
        if condition_results[current_idx]:
            count += 1
            current_idx += 1
        else:
            break
...
@njit
def _consecutive_true_false_calculation(condition_results, m, max_check):
    """JIT 최적화된 연속 True/False 개수 계산"""
    true_count = 0
    false_count = 0
    current_idx = m
    checking_true = True
    
    while current_idx < len(condition_results) and (true_count + false_count) < max_check:
        if current_idx >= len(condition_results):
            break
            
        result = condition_results[current_idx]
...
@njit
def _streak_pattern_calculation(condition_results, pattern, m, max_check):
    """JIT 최적화된 패턴 매칭 계산"""
    ...
    for i, expected in enumerate(pattern):
        ...
        actual = condition_results[current_idx]
...
@njit
def _find_last_condition_break_calculation(condition_results, m, max_check):
    """JIT 최적화된 마지막 조건 깨짐 위치 찾기"""
    last_break_idx = -1
    current_idx = m
    
    while current_idx < len(condition_results) and (current_idx - m) < max_check:
        if not condition_results[current_idx]:
...
@njit
def _atr_calculation(highs, lows, closes, period, m):
    """JIT 최적화된 ATR 계산"""
    if m + period + 1 > len(closes):
        return 0.0
    ...
    for i in range(m, min(m + period, len(closes))):
        if i + 1 >= len(closes):
            break
...
class NumChartManager:
    ...
    def _update_numpy_cache(self):
        ...
        self._np_cache = {
            ...
            'v': np.array([d.get('거래량', 0) for d in raw_data], dtype=np.int32),
            'a': np.array([d.get('거래대금', 0) for d in raw_data], dtype=np.float32)
        }
    ...
    def bar_time(self, n: int = 0) -> str:
        ...
        if not raw_data or n >= len(raw_data):
            return ''
        
        time_str = raw_data[n].get('체결시간', '')
    ...
    def bar_date(self, n: int = 0) -> str:
        ...
        if self.cycle == 'mi':
            ...
            return raw_data[n].get('일자', '')

    # 이동평균 및 통계 함수들 - 벡터화 최적화
    def ma(self, period: int = 20, before: int = 0) -> float:
        ...
        # JIT 함수로 이동평균 계산
        return _ma_calculation(self._np_cache['c'], period, before)
    ...
    def cross_up(self, a_func, b_func) -> bool:
        ...
        Args:
            a_func: 첫 번째 함수 (예: 단기 이평)
            b_func: 두 번째 함수 (예: 장기 이평)
        
        Returns:
            bool: 상향돌파 시 True (이전봉에서 a <= b, 현재봉에서 a > b)
        ...
        사용예:
            ...
            # 현재가가 이동평균선 상향돌파
            signal = cm.cross_up(cm.c, lambda n: cm.ma(20, n))
        """
    ...
    def cross_down(self, a_func, b_func) -> bool:
        ...
        Returns:
            bool: 하향돌파 시 True (이전봉에서 a >= b, 현재봉에서 a < b)
        
        사용예:
            # 5일선이 20일선을 하향돌파
            signal = cm.cross_down(
                lambda n: cm.ma(5, n), 
                lambda n: cm.ma(20, n)
            )
            if signal:
            ...
            # 현재가가 이동평균선 하향돌파
            signal = cm.cross_down(cm.c, lambda n: cm.ma(20, n))
        """
        if not (callable(a_func) and callable(b_func)):
            return False
    ...
    def bars_since(self, condition_func) -> int:
        ...
        Returns:
            int: 조건이 만족된 이후 지나간 봉 개수 (조건 미발견시 전체 데이터 길이)
        
        사용예:
            # 최근 종가가 60000 미만인 봉 이후로 몇 봉이 지났는지
            bars = cm.bars_since(lambda i: cm.c(i) < 60000)
            echo(f"60000 미만 이후 {bars}봉 경과")
            
            # 거래량이 평균의 2배 이상인 봉 이후 경과 봉수
            bars = cm.bars_since(lambda i: cm.v(i) > cm.avg(cm.v, 20, i) * 2)
        """
        self._update_numpy_cache()
        if not self._np_cache['c'].size:
            return 0
    ...
    def highest_since(self, nth: int, condition_func, data_func) -> float:
        ...
        Args:
            nth (int): 몇 번째 조건 만족 시점인지 (1=첫 번째, 2=두 번째...)
            condition_func: 조건을 확인할 함수
            data_func: 최고값을 구할 데이터 함수
        
        Returns:
            float: 해당 시점 이후의 data_func 최고값
        
        사용예:
            # 최근 2번째로 종가가 60000 미만 이후부터 현재까지 고가의 최고값
            high = cm.highest_since(2, lambda i: cm.c(i) < 60000, cm.h)
            echo(f"2번째 60000 미만 이후 최고가: {high}")
            
            # 3번째 급등 이후 거래량 최고값
            max_vol = cm.highest_since(3, 
                lambda i: (cm.c(i) - cm.o(i)) / cm.o(i) > 0.05, 
                cm.v)
        """
        self._update_numpy_cache()
        if not self._np_cache['c'].size:
    ...
    def lowest_since(self, nth: int, condition_func, data_func) -> float:
        ...
        Args:
            nth (int): 몇 번째 조건 만족 시점인지
            condition_func: 조건을 확인할 함수
            data_func: 최저값을 구할 데이터 함수
        
        Returns:
            float: 해당 시점 이후의 data_func 최저값
        
        사용예:
            # 최근 3번째로 종가가 70000 초과 이후부터 현재까지 저가의 최저값
            low = cm.lowest_since(3, lambda i: cm.c(i) > 70000, cm.l)
            echo(f"3번째 70000 초과 이후 최저가: {low}")
        """
        self._update_numpy_cache()
        if not self._np_cache['c'].size:
            return 0.0
    ...
    def value_when(self, nth: int, condition_func, data_func) -> float:
        ...
        Args:
            nth (int): 몇 번째 조건 만족 시점인지
            condition_func: 조건을 확인할 함수
            data_func: 값을 구할 데이터 함수
        
        Returns:
            float: 해당 시점의 data_func 값
        
        사용예:
            # 최근 3번째로 종가가 60000을 넘은 시점의 시가
            open_price = cm.value_when(3, lambda i: cm.c(i) > 60000, cm.o)
            echo(f"3번째 60000 초과시 시가: {open_price}")
            
            # 5번째 거래량 급증 시점의 종가
            close_price = cm.value_when(5, 
                lambda i: cm.v(i) > cm.avg(cm.v, 20, i) * 3, 
                cm.c)
        """
        self._update_numpy_cache()
        if not self._np_cache['c'].size:
    ...
    def top_volume_avg(self, n: int = 128, cnt: int = 10, m: int = 1) -> float:
        ...
        # JIT 함수로 상위값 선택
        return _top_selection_calculation(volumes, cnt)
    ...
    def top_amount_avg(self, n: int = 128, cnt: int = 10, m: int = 1) -> float:
        ...
        # JIT 함수로 상위값 선택
        return _top_selection_calculation(amounts, cnt)
    
    def get_obv_array(self, count: int = 10) -> list:
        """OBV 배열 - JIT 최적화"""
        self._update_numpy_cache()
        if not self._np_cache['c'].size or self._data_length < 2:
            return [0.0] * count
        
        # numpy 배열로 데이터 준비
        actual_count = min(count, self._data_length)
        closes = self._np_cache['c'][:actual_count]
        volumes = self._np_cache['v'][:actual_count]
        
        if actual_count < 2:
            return [0.0] * count
        
        # JIT 함수로 OBV 계산
        obv_values = _obv_calculation(closes, volumes, count)
        
        return obv_values.tolist()
    ...
    def get_extremes(self, n: int = 128, m: int = 1) -> dict:
        ...
        # JIT 함수로 극값 계산
        hh, hc, lc, ll, hv, lv, ha, la = _extremes_calculation(highs, closes, lows, volumes, amounts, n, m)
        ...
        result = {
            ...
            'ha': float(ha),
            'la': float(la),
            'close': closes[min(m + 1, len(closes) - 1)] if len(closes) > m + 1 else 0,
            'bars': m + 1
        }
    ...
    def get_close_tops(self, n: int = 128, cnt: int = 80, m: int = 1) -> tuple:
        ...
        self._update_numpy_cache()
        if not self._np_cache['c'].size or m < 0 or n <= 0 or cnt <= 0:
            return ([], 0)
        ...
        # JIT 함수로 최고종가 감지
        high_close_indices = _close_tops_detection(self._np_cache['c'], n, cnt, m)
        
        return (high_close_indices, today_bars)

    def consecutive_count(self, condition_func, m: int = 0, max_check: int = 128) -> int:
        """연속 조건 만족 봉 개수 - JIT 최적화"""
        self._update_numpy_cache()
        if not callable(condition_func):
            return 0
        
        # 조건 결과를 미리 계산하여 배열로 변환
        condition_results = []
        for i in range(m, min(m + max_check, self._data_length)):
            try:
                result = condition_func(i)
                condition_results.append(result)
            except:
                condition_results.append(False)
        
        # JIT 함수로 연속 개수 계산
        return _consecutive_count_calculation(condition_results, 0, max_check)

    def consecutive_true_false(self, condition_func, m: int = 0, max_check: int = 100) -> tuple:
        """연속 True/False 개수 - JIT 최적화"""
        self._update_numpy_cache()
        if not callable(condition_func):
            return (0, 0)
        ...
        # JIT 함수로 연속 True/False 개수 계산
        return _consecutive_true_false_calculation(condition_results, 0, max_check)

    def streak_pattern(self, condition_func, pattern: str, m: int = 0, max_check: int = 100) -> bool:
        """특정 패턴이 연속으로 나타나는지 확인 - JIT 최적화"""
        self._update_numpy_cache()
        if not callable(condition_func) or not pattern:
            return False
        ...
        # 조건 결과를 미리 계산하여 배열로 변환
        condition_results = []
        for i in range(m, m + pattern_length):
        ...
        # JIT 함수로 패턴 매칭 계산
        return _streak_pattern_calculation(condition_results, pattern, 0, max_check)

    def find_last_condition_break(self, condition_func, m: int = 0, max_check: int = 128) -> int:
        """조건이 마지막으로 깨진 위치 찾기 - JIT 최적화"""
        self._update_numpy_cache()
        if not callable(condition_func):
            return -1
        ...
        # JIT 함수로 마지막 조건 깨짐 위치 찾기
        return _find_last_condition_break_calculation(condition_results, 0, max_check)
    ...
    def rsi(self, period: int = 14, m: int = 0) -> float:
        ...
        # JIT 함수로 RSI 계산
        return _rsi_calculation(self._np_cache['c'], period, m)
    ...
    def atr(self, period: int = 14, m: int = 0) -> float:
        """평균 실제 범위(ATR) 계산 - JIT 최적화"""
        self._update_numpy_cache()
        if not self._np_cache['c'].size or self._data_length < period + 1 + m:
            return 0.0
        
        # JIT 함수로 ATR 계산
        return _atr_calculation(self._np_cache['h'], self._np_cache['l'], self._np_cache['c'], period, m)
    ...
    def is_engulfing(self, n: int = 0, bullish: bool = True) -> bool:
        """포괄 패턴 확인"""
        self._update_numpy_cache()
        if not self._np_cache['c'].size or n + 1 >= self._data_length:
            return False
    ...
    # 스크립트 함수들 (매매 스크립트에서 주로 사용)
    def bar(self, n: int = 0) -> tuple:
        """
        봉 데이터 반환
        
        Args:
            n (int): 봉 인덱스 (0=현재봉, 1=1봉전...)
        
        Returns:
            tuple: (시가, 고가, 저가, 종가, 거래량, 거래대금)
        
        사용예:
            o, h, l, c, v, a = cm.bar(0)  # 현재봉 모든 데이터
            prev_bar = cm.bar(1)          # 1봉전 데이터
        """
        self._update_numpy_cache()
        if not self._np_cache['c'].size or n >= self._data_length:
            return (0, 0, 0, 0, 0, 0)
        
        return (self.o(n), self.h(n), self.l(n), self.c(n), self.v(n), self.a(n))

    def longest_bar(self, p: float = 2.0, n: int = 0) -> tuple:
        """
        당일 가장 긴 양봉 찾기 (분봉만 해당)
        
        Args:
            p (float): 최소 상승률(%) 기준 (기본값: 2.0%)
            n (int): 검사 기준날짜 봉 인덱스 (0=현재봉)
        
        Returns:
            tuple: (위치, 시간, 시가, 고가, 저가, 종가, 거래량, 거래대금)
                   조건 미충족시 (0, '', 0, 0, 0, 0, 0, 0)
        
        사용예:
            pos, time, o, h, l, c, v, a = cm.longest_bar(3.0)  # 3% 이상 상승봉
            if pos > 0:
                echo(f"가장 긴 봉: {time} {c-o}원 상승")
        """
        if self.cycle != 'mi':
        ...
        raw_data = self.cht_dt._chart_data.get(self.code, {}).get(f'mi{self.tick}', [])
        date_str = raw_data[n].get('체결시간', '')[:8]
        
        length = 0
        pos = 0
        
        for i in range(n, len(raw_data)):
            if raw_data[i].get('체결시간', '')[:8] != date_str:
                break
            
            diff = self.c(i) - self.o(i)
            if diff > length:
                length = diff
                pos = i
        
        if pos > 0 and (length / self.o(pos)) * 100 >= p:
            return (pos, raw_data[pos].get('체결시간', ''), self.o(pos), 
                   self.h(pos), self.l(pos), self.c(pos), self.v(pos), self.a(pos))
    ...
    def past_bars(self, dt: str = None) -> int:
        ...
        Args:
            dt (str): 날짜 (YYYYMMDD), None시 오늘
        
        Returns:
            int: 해당 날짜의 분봉 개수
        
        사용예:
            today_bars = cm.past_bars()           # 오늘 분봉 개수
            bars_0101 = cm.past_bars('20250101')  # 특정일 분봉 개수
            if today_bars > 100:
                echo("오늘 100봉 이상 거래됨")
        """
        if self.cycle != 'mi':
    ...
    def get_extremes(self, n: int = 128, m: int = 1) -> dict:
        ...
        Args:
            n (int): 검사할 봉 개수 (기본값: 128)
            m (int): 시작 봉 인덱스 (기본값: 1, 1봉전부터)
        
        Returns:
            dict: {
                'hh': 최고고가, 'hc': 최고종가, 'lc': 최저종가, 'll': 최저저가,
                'hv': 최고거래량, 'lv': 최저거래량, 'ha': 최고거래대금, 'la': 최저거래대금,
                'close': 전일종가, 'bars': 당일봉수
            }
        
        사용예:
            extr = cm.get_extremes(100, 1)  # 1봉전부터 100봉 구간
            if cm.c() > extr['hc']:
                echo(f"신고가 갱신! {cm.c()} > {extr['hc']}")
        """
        self._update_numpy_cache()
        if not self._np_cache['c'].size or n <= 0:
            return {'hh': 0, 'hc': 0, 'lc': 0, 'll': 0, 'hv': 0, 'lv': 0, 'ha': 0, 'la': 0, 'close': 0, 'bars': 0}
        ...
        result = {
            'hh': int(np.max(highs)),
            'hc': int(np.max(closes)),
            'lc': int(np.min(closes)),
            'll': int(np.min(lows)),
            'hv': int(np.max(volumes)),
            'lv': int(np.min(volumes)),
            'ha': float(np.max(amounts)),
            'la': float(np.min(amounts)),
    ...
    def top_volume_avg(self, n: int = 128, cnt: int = 10, m: int = 1) -> float:
        ...
        사용예:
            top10_avg = cm.top_volume_avg(128, 10, 1)  # 1봉전부터 128봉중 상위10개 평균
            if cm.v() > top10_avg * 0.8:
                echo(f"거래량 급증: {cm.v()} > {top10_avg*0.8}")
        """
        self._update_numpy_cache()
        if not self._np_cache['v'].size or n <= 0 or cnt <= 0 or m < 0:
            return 0.0
        ...
        return float(np.mean(top_volumes))
    ...
    def top_amount_avg(self, n: int = 128, cnt: int = 10, m: int = 1) -> float:
        ...
        사용예:
            top10_amt = cm.top_amount_avg(128, 10, 1)  # 상위10개 거래대금 평균
            if cm.a() > top10_amt:
                echo(f"거래대금 상위권: {cm.a()}")
        """
        self._update_numpy_cache()
        if not self._np_cache['a'].size or n <= 0 or cnt <= 0 or m < 0:
            return 0.0
        ...
        return float(np.mean(top_amounts))

    def get_close_tops(self, n: int = 128, cnt: int = 80, m: int = 1) -> tuple:
        """
        각 봉이 자신을 포함한 cnt개 봉 중 최고 종가인지 확인하여 인덱스 수집 (분봉만 해당)
        
        Args:
            n (int): 검사 시작 기준 (130이면 129+m부터 시작)
            cnt (int): 비교할 봉 개수 (자신 포함)
            m (int): 검사 종료 인덱스 (0=현재봉까지, 1=1봉까지...)
        
        Returns:
            tuple: (최고종가_인덱스_리스트, 당일_봉_개수)
        
        사용예:
            gcts = cm.get_close_tops(128, 128, 1)  # 128봉 기준으로 최고종가 찾기
            최고종가 = gcts[0]  # 최고종가 인덱스 리스트
            당일봉수 = gcts[1]   # 당일 봉 개수
            if len(최고종가) > 1:
                echo(f"최고종가 {len(최고종가)}개 발견")
        """
        if self.cycle != 'mi':
            return ([], 0)
        ...
        # 당일 봉 개수 계산
        today_bars = self._calculate_today_bars()
        
        # n-1+m부터 m까지 역순으로 검사 (데이터 길이 고려)
        start_idx = min(n - 1 + m, self._data_length - 1)
        end_idx = m
        ...
        # numpy 벡터화로 최고종가 감지 (정확한 인덱스 계산)
        for current_idx in range(start_idx, end_idx - 1, -1):
            if current_idx >= self._data_length:
                continue
            
            current_close = self._np_cache['c'][current_idx]
            
            # 비교 범위 계산
            compare_start = current_idx
            compare_end = min(current_idx + cnt, self._data_length)
            
            if compare_start >= compare_end:
                continue
            
            # numpy 슬라이싱으로 최대값 계산 (벡터화)
            compare_closes = self._np_cache['c'][compare_start:compare_end]
            max_close = np.max(compare_closes)
    ...
    def _calculate_today_bars(self) -> int:
        """당일 봉 개수 계산 (numpy 벡터화)"""
        today = datetime.now().strftime('%Y%m%d')
        raw_data = self.cht_dt._chart_data.get(self.code, {}).get(f'mi{self.tick}', [])
    ...
    def consecutive_count(self, condition_func, m: int = 0, max_check: int = 128) -> int:
        ...
        Args:
            condition_func: 조건을 확인할 함수 (인덱스를 받아 bool 반환)
            m (int): 시작 기준 봉 (0=현재봉부터, 1=1봉전부터...)
            max_check (int): 최대 확인할 봉 개수 (무한루프 방지)
        ...
        사용예:
            ...
            echo(f"연속 상승 {count}봉")
            
            # 1봉전부터 거래량이 평균보다 높은 봉 개수
    ...
    def consecutive_true_false(self, condition_func, m: int = 0, max_check: int = 100) -> tuple:
        ...
        Args:
            condition_func: 조건을 확인할 함수
            m (int): 시작 기준 봉
            max_check (int): 최대 확인할 봉 개수
        
        Returns:
            tuple: (연속_True_개수, 연속_False_개수)
        
        사용예:
            true_cnt, false_cnt = cm.consecutive_true_false(lambda i: cm.c(i) > cm.c(i+1))
            echo(f"연속상승 {true_cnt}봉 후 연속하락 {false_cnt}봉")
        """
        self._update_numpy_cache()
        if not callable(condition_func):
            return (0, 0)
    ...
    def streak_pattern(self, condition_func, pattern: str, m: int = 0, max_check: int = 100) -> bool:
        ...
        Args:
            condition_func: 조건을 확인할 함수
            pattern (str): 확인할 패턴 ('T'=True, 'F'=False) 예: "TTTFF", "TFTF"
            m (int): 시작 기준 봉
            max_check (int): 최대 확인할 봉 개수
    ...
    def find_last_condition_break(self, condition_func, m: int = 0, max_check: int = 128) -> int:
        ...
        사용예:
            # 상승 조건이 마지막으로 깨진 위치
            break_pos = cm.find_last_condition_break(lambda i: cm.c(i) > cm.c(i+1))
            if break_pos >= 0:
                echo(f"{break_pos}봉에서 상승 조건 마지막 break")
        """
        self._update_numpy_cache()
        if not callable(condition_func):
            return -1
    ...
    def get_ma(self, period: int = 20, count: int = 1) -> list:
        """이동평균 리스트 반환"""
        self._update_numpy_cache()
        if not self._np_cache['c'].size or self._data_length < period:
            return []
        
        ma_list = []
        for i in range(count):
            if i + period > self._data_length:
                break
            
            closes = self._np_cache['c'][i:i + period]
            ma_list.append(float(np.mean(closes)))
