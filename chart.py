...
class ChartData:
    ...
    def set_chart_data(self, code: str, data: list, cycle: str, tick: int = 1):
        """외부에서 차트 데이터 설정 - 1분봉과 일봉만 허용"""
        ...
        if cycle == 'mi' and tick != 1:
            logging.warning(f"[{datetime.now()}] Only 1-minute data allowed for minute cycle. Rejected: {code} cycle={cycle}, tick={tick}")
            return
        elif cycle not in ['mi', 'dy']:
            logging.warning(f"[{datetime.now()}] Only 'mi'(tick=1) and 'dy' cycles allowed. Rejected: {code} cycle={cycle}")
            return
        ...
        with lock:
            ...
            if cycle == 'mi' and tick == 1:
                # 1분봉 설정
                chart_data['mi1'] = data
                self._create_index_map(chart_data, code, 'mi1')
                ...
                # 일봉 설정
                chart_data['dy'] = data
                self._create_index_map(chart_data, code, 'dy')
                
                # 주봉, 월봉 자동 생성
                chart_data['wk'] = self._aggregate_day_data(data, 'week')
                self._create_index_map(chart_data, code, 'wk')
                
                chart_data['mo'] = self._aggregate_day_data(data, 'month')
                self._create_index_map(chart_data, code, 'mo')
    ...
    def update_chart(self, code: str, price: int, volume: int, amount: int, datetime_str: str):
        ...
        with lock:
            ...
            # 1분봉 업데이트 로직 (기존과 동일)
            self._update_minute_chart(chart_data, code, price, volume, amount, datetime_str, base_time)
            ...
            if 'dy' in chart_data and chart_data['dy']:
                ...
                # 일봉이 업데이트되면 주봉/월봉도 업데이트
                self._update_week_month_chart(chart_data, code, price, volume, amount, datetime_str)
        ...
        elapsed = time.time() - start_time
        if elapsed > 0.01:
            logging.debug(f"[{datetime.now()}] update_chart for {code} passed {self.pass_count} times took {elapsed:.6f} seconds")
            self.pass_count = 0

    def _update_minute_chart(self, chart_data, code, price, volume, amount, datetime_str, base_time):
        """1분봉 업데이트 로직 분리"""
        minute_data = chart_data['mi1']
    ...
    def _update_week_month_chart(self, chart_data, code, price, volume, amount, datetime_str):
        """주봉/월봉 업데이트 (일봉 업데이트 후 호출)"""
        ...
        # 주봉 업데이트
        self._update_period_chart(chart_data, code, price, volume, amount, date_obj, 'wk', 'week')
        
        # 월봉 업데이트
        self._update_period_chart(chart_data, code, price, volume, amount, date_obj, 'mo', 'month')

    def _update_period_chart(self, chart_data, code, price, volume, amount, date_obj, cycle_key, period_type):
        """주봉/월봉 공통 업데이트 로직"""
        ...
        # 인덱스 맵 사용하여 현재 주기 데이터 빠르게 찾기
        if ('index_maps' in chart_data and 
            cycle_key in chart_data['index_maps'] and 
        ...
        # 새 데이터 인덱스 추가
        chart_data['index_maps'][cycle_key][current_period_key] = 0
        
    def get_chart_data(self, code: str, cycle: str, tick: int = None) -> list:
        """특정 종목, 주기의 차트 데이터 반환 (최적화된 버전)"""
        start_time = time.time()
        ...
        with lock:
            ...
            if cycle == 'mi' and tick == 1 and 'mi1' in chart_data and chart_data['mi1']:
                self._update_latest_candle_from_memory(chart_data, code)
            
            cycle_key = cycle if cycle != 'mi' else f'mi{tick}'
            
            # 저장된 데이터가 있으면 바로 반환 (가장 빠른 경우)
            if cycle_key in chart_data and chart_data[cycle_key]:
                result = chart_data[cycle_key].copy()
                
                elapsed = time.time() - start_time
                if elapsed > 0.01:
                    logging.debug(f"[{datetime.now()}] get_chart_data (cached) for {code} took {elapsed:.6f} seconds")
            ...
            if cycle == 'mi' and 'mi1' in chart_data and chart_data['mi1']:
                ...
                # 집계 데이터 저장 (다음 요청을 위해 캐싱)
                chart_data[cycle_key] = aggregated_data
                self._save_code_data(code, chart_data)
                
                elapsed = time.time() - start_time
                if elapsed > 0.01:
                    logging.debug(f"[{datetime.now()}] get_chart_data (aggregated) for {code} took {elapsed:.6f} seconds")
    ...
    def _update_latest_candle_from_memory(self, chart_data, code):
        ...
        try:
            ...
            logging.debug(f"[{datetime.now()}] Error updating latest candle: {str(e)}", exc_info=True)
