...
class SIMServer():
   ...
   def api_init(self):
      try:
         if self.sim_no > 1:  # 키움서버 사용 (sim_no=2, 3)
            logging.debug(f'{self.name} api_init start (Sim mode {self.sim_no})')
   ...
   def _set_signal_slots(self):
      if self.sim_no != 1 and self.ocx:  # sim_no=2,3일 때만
   ...
   def SetRealReg(self, screen, code_list, fid_list, opt_type):
      ...
      if screen not in real_thread:
         if self.sim_no in [1, 2]:  # sim_no=1,2일 때
            thread = OnReceiveRealDataSim1And2()
         else:  # sim_no=3일 때
            thread = OnReceiveRealDataSim3()
   ...
   def SendConditionStop(self, screen, cond_name, cond_index):
      global cond_thread
      if self.sim_no != 1 and self.ocx:  # 키움서버 사용 (sim_no=2, 3)
