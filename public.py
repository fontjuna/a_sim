...
class TableColumns:
    ...
    hd주문목록 = {
        ...
        '정수': ['주문수량', '주문가격', '미체결수량'],
        '실수': [],
        '컬럼': ['키', '전략', '구분', '상태', '종목코드', '종목명', '주문번호', '주문수량', '주문가격', '미체결수량', '요청명', '비고'],
...
class GlobalMemory:
    ...
    전략설정 = None # json
    전략쓰레드 = None
    매수문자열들 = [''] * 6                # ['000 : 전략01', '', ...] # SendConditionStop 에서 사용, OnReceiveRealCondition 에서 쓰레드 분기
    매도문자열들 = [''] * 6                # ['000 : 전략01', '', ...] # SendConditionStop 에서 사용, OnReceiveRealCondition 에서 쓰레드 분기
    dict잔고종목감시 = {}
    dict조건종목감시 = {}
